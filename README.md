...
Dramatic Sky Ride adds controllable **Flight, Ground Ride and Visible Surf mounts** to **Gen1Recomp**, with native 2D flight, optional voxel cameras, Generation II mounts, Wild Skies integration, and support for either Wilds of Kanto or PokéPC Followers.
...
For the intended Pokémon mount artwork and follower integration, however, install **exactly one** provider below.
...
| 3D voxel world / 1ST / 3RD cameras | **Battle Art Voxel Fork** or **Dramaless Shape** | Optional |
...
Use the setup above and add **one** voxel provider:
...
Neither is required for native 2D flight. Battle Art is preferred automatically if both are installed, but a normal setup only needs one.

The optional Stadium renderer remains experimental and is intentionally **not** part of the recommended 0.1.6 setup.
...
- **Wild Skies** is strongly recommended for visible airborne Pokémon and exact intercepted species/level battles.
- **Flying Music** can reuse installed FRLG, HGSS or LGPE Surf/Bike tracks without redistributing audio.
- Compatibility guards protect active mount logic when Wilds or companion mods rebuild overworld hooks.
...
- **Optional voxel:** Battle Art Voxel Fork or Dramaless Shape.
