;(function()
-- -------------------------------------------------------------------------
-- Gen 2 Open Sky runtime guard.
--
-- The first playable Open Sky layer intentionally reused pieces of Gold's
-- Pokegear renderer. That path is convenient but it also means a small API or
-- asset-layout difference in a Gen1Recomp++ build can throw from the very first
-- regional frame. A thrown state callback is fatal to the LOVE process.
--
-- This guard sits AFTER main_53c. It protects entry/update and replaces the
-- experimental regional draw path with a dependency-light renderer until the
-- native Town Map integration has been runtime-verified on real Gen2 builds.
-- A failure now returns to DSR local cruise instead of closing the game.
-- -------------------------------------------------------------------------
...
local function rememberError(kind, err)
  ...
  pcall(function()
    log("Open Sky runtime guard caught %s error: %s",
      lastRuntimeError.kind, lastRuntimeError.message)
  end)
end

local function safeNotice(text, seconds)
  pcall(function() notifyHud(text, seconds or 2.0) end)
end
...
local function recoverToLocal(state, reason)
  -- Keep the player airborne and below the Open Sky entry threshold. This is
  -- deliberately conservative: the authoritative local map never moved unless
  -- the player explicitly chose a regional descent in main_53c.
  flight.active = true
  flight.phase = "cruise"
  ...
  if type(openSkyApi.leave) == "function" then
    pcall(openSkyApi.leave, reason or "runtime_guard")
  end
  popIfCurrent(state)
...
local function safeDraw(state)
  ...
  local ok, err = pcall(function()
    ...
    -- Dependency-light regional view. Every operation below is a core LOVE
    -- primitive already used by Gen1Recomp itself; no Pokegear/Font/TileSheet
    -- internals are touched on this guarded path.
    G.setColor(0.58, 0.80, 0.96, 1)
    G.rectangle("fill", 0, 0, 160, 144)

    G.setColor(0.22, 0.46, 0.29, 0.75)
    for _, anchor in ipairs(stableAtlas(state.region)) do
      local x, y = tonumber(anchor.x), tonumber(anchor.y)
      if x and y then G.circle("fill", x, y, 1.5) end
    end
    ...
    local x = tonumber(state.x) or 80
    local y = tonumber(state.y) or 72
    G.setColor(1, 1, 1, 1)
    ...
    if type(G.print) == "function" then
      ...
      if not bottom then
        ...
        bottom = name and ("A: DESCEND  " .. tostring(name))
          or "FLY TO A VISITED LANDING POINT"
      end
  ...
  if not ok then
    rememberError("draw", err)
    -- Drawing errors are kept non-fatal. Update remains live, so B or descent
    -- can still leave the state; the next tick also has the protected updater.
  end
end
...
local function patchState(state)
  ...
  if type(rawUpdate) == "function" then
    state.update = function(self, dt)
      ...
      rememberError("update", result)
      recoverToLocal(self, "runtime_update_error")
      return nil
    end
  end

  -- Do not call the experimental main_53c renderer here. The navigation and
  -- descent logic stay unchanged; only presentation is temporarily isolated.
...
local function patchCurrentState()
  ...
  if state then patchState(state) end
end

-- Protect the exact transition frame too. main_53c pushes the Open Sky state
-- from inside the previous OverworldState:update wrapper; a screen.pushed hook
-- or constructor mismatch can therefore throw before we ever reach state:update.
local previousGuardedOpenSkyUpdate = OverworldState.update
function OverworldState:update(dt, ...)
  local extras = { ... }
  local unpackArgs = table.unpack or unpack
  local ok, result = pcall(function()
    return previousGuardedOpenSkyUpdate(self, dt, unpackArgs(extras))
  end)
  if not ok then
    rememberError("entry", result)
    recoverToLocal(currentState(), "runtime_entry_error")
    return nil
  end
  patchCurrentState()
  return result
